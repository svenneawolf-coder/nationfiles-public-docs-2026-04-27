# NationFiles JSON Export Sample - /en/
...
This endpoint provides global export data used in high-level NationFiles dashboards. The sampled payload was fetched from path '/en/' with query 'export=json&chart=world_index_30d'. The payload type is object with 1 top-level entries. This sample is intentionally truncated (no pagination) to provide a stable, lightweight download artifact for documentation, QA checks, and integration previews. Representative key paths include: world_index_30d, world_index_30d.labels, world_index_30d.labels[], world_index_30d.values, world_index_30d.values[].
...
- **Public source URL**: https://nationfiles.com/en/?export=json&chart=world_index_30d
...
## Key-Value Semantics
- `world_index_30d`: Calculated score/index value.
- `world_index_30d.labels`: Array/list of related values.
- `world_index_30d.labels[]`: Array/list of related values.
- `world_index_30d.values`: Primary metric value.
- `world_index_30d.values[]`: Primary metric value.
